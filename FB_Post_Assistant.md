...
**Một khả năng va chạm KHÁC vẫn CHƯA xử lý, ghi nhận lại theo yêu cầu chủ dự án (2026-09-10, chưa sửa):** khoảng cách enforcement thật giữa 2 comment liên tiếp không phải do lịch quyết định — mỗi khi một comment đăng THẬT xong, `safety.py`'s `record()` tự random một khoảng gap MỚI (gọi tắt x_safety, trong `[comment_min_delay_seconds, comment_max_delay_seconds]`) làm mốc `next_allowed_at` cho comment kế tiếp — độc lập hoàn toàn với con số random đã dùng để lên lịch (`x₁`) từ trước đó. Vì x_safety chỉ tồn tại SAU KHI comment 1 đăng xong (một sự kiện ở tương lai tại thời điểm lên lịch), không có cách nào biết trước để "kẹp sàn" tránh va chạm với nó — nếu `x₁ < x_safety`, comment 2 tới giờ lên lịch vẫn bị `safety.py` chặn, phải đợi thêm (không lỗi, chỉ trễ — `fire_due_tasks()` tự dò lại mỗi phút cho tới khi qua giờ chặn, xem `rate_limit_wait_message()`/`last_warning` trên task).

Có 2 hướng đã bàn nhưng CHƯA làm: (A) chấp nhận, để cơ chế tự dò lại xử lý (hiện trạng, không cần sửa gì) — trễ giờ đăng thật một chút nhưng không mất task; (B) gộp 2 lớp random thành 1 (lưu sẵn gap dùng để lên lịch ngay trên task, để `record()` đọc lại thay vì tự random mới) — loại bỏ va chạm hoàn toàn nhưng đụng kiến trúc. Một đề xuất khác từng cân nhắc — cố định `x_safety = comment_min_delay_seconds` thay vì random — đảm bảo `x₁ ≥ x_safety` luôn đúng (vì x₁ vốn random trong `[min, max]`) nên loại bỏ va chạm 100% cho đường đi qua `data_sync.py`'s scheduler, nhưng đánh đổi: các đường KHÔNG qua scheduler (vd nút "Đăng ngay" ở `/admin/schedule`) sẽ luôn có khoảng cách enforcement CỐ ĐỊNH — đúng kiểu "y hệt nhau mọi lần" mà [rate-limiting-pacing.md](docs/skills/rate-limiting-pacing.md) rule #3 khuyến cáo tránh. Rủi ro này được đánh giá là nhỏ (đường "Đăng ngay" do người thật bấm tay, không phải vòng lặp tự động lặp lại) nhưng **chưa quyết định triển khai**, cần chủ dự án chốt hướng sau.
