...
| `post_to_group(page, group_url, content, media_path?)` | Publish a new post inside a specific Facebook group | **All 4 tiers implemented** (Tier 4 direct URL confirmed live 2026-09-04; Tiers 1-3 — pinned shortcut, joined-groups list, search — added since, see `_tier1_pinned_shortcut`/`_tier2_your_groups`/`_tier3_search` in `human_bot/actions.py`) — no audience/privacy step (unlike `post_to_own_profile`), visibility follows the group's own settings. **See `skills/group-targeting.md`** for the 4-tier navigation fallback chain. Pending-approval detection text/timing confirmed via Codegen 2026-09-10 (`codegen_verify_pending_approval.py`) — still missing one last step: a real `post_to_group` task fired through the bot itself (not Codegen by hand) into a group with approval on, to confirm `result.txt` actually reads `posted_to_group_pending_approval` end-to-end. |
